...
\begin{table}[ht]
\centering
\caption[Singapore component breakdown for the cross-city DiffusionDrive matrix.]{Singapore destination component scores for every cross-city DiffusionDrive row whose training city is not Singapore. Each row is one trained model evaluated on Singapore. Values are percentages for the five PDMS sub-scores used in the main-text aggregate. The table is grouped by training city; within each group, the supervised ResNet-34 row is the comparison anchor and the frozen SSL rows follow. The pattern is consistent across training cities: Singapore transfer failure concentrates in DAC and EP (geometry-sensitive sub-scores) while NC and TTC (collision-adjacent sub-scores) remain stable across backbones.}

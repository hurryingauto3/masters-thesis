...
\begin{table}[ht]
    \centering
    \caption{%
    NAVSIM city-split data card. Logs were grouped by nuPlan \texttt{map\_location} tag. \emph{Trainval} is the pool used for both training and validation under the cross-city benchmark's single-city and all-city protocols; \emph{test} is the held-out navtest subset used for every cross-city evaluation. Scene count = log count because each log contributes a single NAVSIM scene.%
    }

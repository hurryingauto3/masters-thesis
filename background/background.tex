...
TransFuser is a representative multi-modal end-to-end planner that fuses camera and LiDAR features through transformer-style cross-modal attention before regressing a future trajectory \cite{chitta-transfuser-2023,prakash-transfuser-2021}. The model processes each modality with its own convolutional backbone, typically ResNet-34 for both streams in NAVSIM. It inserts transformer blocks at four resolution scales to allow image and LiDAR tokens to attend to one another, aggregates the fused feature map into a compact scene embedding, and decodes a sequence of future waypoints with a GRU-based autoregressive head conditioned on ego status and a discrete driving command \cite{chitta-transfuser-2023}. Published TransFuser and Latent TransFuser baselines on NAVSIM \texttt{navtest}, and the authors' reference training recipe, are summarized in \reftab{tab:bg-transfuser-baseline} and \reftab{tab:bg-transfuser-recipe}.

\begin{table}[ht]
\centering
\caption{Published TransFuser and Latent TransFuser baselines on \texttt{navtest}~\cite{dauner-navsim-2024}. Single-run standard deviation across training seeds is $\pm 0.6$ PDMS points; the $0.2$-point TF--LTF gap is within this band.}
\label{tab:bg-transfuser-baseline}
\small
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}l c c c c c c@{}}
\toprule
\textbf{Planner} & \textbf{Modality} & \textbf{PDMS (\%)} & \textbf{NC (\%)} & \textbf{DAC (\%)} & \textbf{TTC (\%)} & \textbf{EP (\%)} \\
\midrule
TransFuser        & Camera $+$ LiDAR & $84.0$ & $97.7$ & $92.8$ & $92.8$ & $79.2$ \\
Latent TransFuser & Camera only      & $83.8$ & $97.4$ & $92.8$ & $92.4$ & $79.0$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{TransFuser / LTF reference training recipe~\cite{dauner-navsim-2024}. Both agents are trained with this recipe on \texttt{navtrain}.}
\label{tab:bg-transfuser-recipe}
\small
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{4.0cm} >{\raggedright\arraybackslash}p{8.0cm}@{}}
\toprule
\textbf{Setting} & \textbf{Value} \\
\midrule
Image encoder    & ResNet-34 \\
Cameras          & 3 front-facing, stitched ($\approx\!140^{\circ}$ FOV) \\
Input resolution & $1024 \times 256$ \\
Optimizer        & Adam, constant LR $1{\times}10^{-4}$ \\
Batch size       & $96$ \\
Epochs           & $100$ \\
Hardware         & 1 $\times$ NVIDIA A5000 ($\approx\!1$ day) \\
\bottomrule
\end{tabular}
\end{table}

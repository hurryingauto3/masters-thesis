...
This thesis answers three linked questions rather than a single benchmark objective. First, self-supervised backbones can improve even lightweight end-to-end planners. Second, pooled mixed-city PDMS is not the setting where their value is most visible. Third, under explicit city-disjoint evaluation, the value of frozen SSL backbones is cross-city robustness, together with a bounded representational mechanism that helps explain it.
...
\begin{enumerate}
    ...
    \item \textbf{Cross-city SSL generalisation}. Under the evaluated settings with LAW open-loop on nuScenes and TransFuser / Latent TransFuser closed-loop on NAVSIM, frozen self-supervised representations (I-JEPA, DINOv2, MAE) reduce cross-city generalisation gaps relative to supervised baselines. The conclusion is direct: \textbf{cross-city failure is strongly mediated by the visual representation.}

    \item \textbf{DiffusionDrive $+$ SSL}. SSL backbones are integrated into DiffusionDrive through the same feature adapter used in the cross-city benchmark, including mixed-city camera-only rows. The mixed-city integration shows that the representation signal survives a planner swap under pooled evaluation. The $4 \times 4 \times 4$ city-disjoint camera$+$LiDAR matrix shows that it survives under city shift as well: every frozen self-supervised family produces a smaller mean out-of-distribution PDMS drop than supervised ResNet-34 (DINOv2 by $1.7$ points, MAE by $1.6$, I-JEPA by $3.3$), the per-training-city criterion pre-registered in \refsec{sec:meth-dd-success} is met, and the Singapore-trained row shows that frozen SSL representations resist single-city overfitting under generative planning where the supervised baseline does not.

    \item \textbf{Mechanistic analysis}. A linear-probe, CKA, MMD, and feature-covariance effective-rank study of the trained checkpoint roster. The three calibrated findings are: (a) on matched Boston-only Latent TransFuser rows, the linear-probe ordering tracks the out-of-distribution PDMS drop ordering reported in \reftab{tab:probe-boston}; (b) fine-tuning compresses feature-covariance rank for every self-supervised family; (c) the signal is strongest within matched comparisons and attenuates when backbone scale, pretraining data, and training city vary together. CKA and MMD are supporting diagnostics. No universal-mechanism claim is made.
\end{enumerate}
...
The thesis-level takeaway is that lightweight-head experiments motivate the representation hypothesis, Drive-JEPA demonstrates the in-distribution ceiling on pooled NAVSIM, and zero-shot cross-city transfer is the regime where representation choice matters most.
...
\begin{enumerate}
    ...
    \item \textbf{The in-distribution cost is small; the out-of-distribution gain is large.} The DiffusionDrive all-city table shows at most a small mixed-city cost for SSL, while the cross-city LAW and TransFuser / Latent TransFuser matrices show a materially larger transfer gain under explicit geographic shift. The cross-city DiffusionDrive matrix shows the same direction under a planner whose anchor-based head was selected for its resistance to backbone variation: the absolute gap is compressed but the ordering survives, with the largest single-cell improvement at $5.2$ points of Singapore-drop reduction (DINOv2 trained on Pittsburgh).
    ...
    \item \textbf{In the evaluated settings, representation choice mediates transfer more strongly than head choice.} The same ordinal pattern appears under LAW open-loop on nuScenes, TransFuser and Latent TransFuser closed-loop on NAVSIM, and DiffusionDrive both in mixed-city pooled evaluation and in the $4 \times 4 \times 4$ city-disjoint matrix. The pattern survives the planner whose head was selected for its resistance to backbone variation, which supports the planner-invariant version of the claim within the planners evaluated in this thesis.
\end{enumerate}
...
The practical implication of the thesis is straightforward. A strong pooled benchmark score does not guarantee that an end-to-end planner will remain reliable when the operating city changes. For deployment teams, geographic holdout evaluation should be part of model validation rather than an optional add-on.

The results also suggest a concrete design preference. When transfer to a new city is likely, a frozen self-supervised backbone is often the safer operating point than a supervised baseline that is slightly stronger on the pooled benchmark. The transfer gains are larger than the in-distribution losses, and the frozen setting avoids the feature-rank collapse that appears under aggressive fine-tuning.
...
\begin{itemize}
    ...
    \item \textbf{Camera-only cross-city DiffusionDrive is not evaluated.} The cross-city DiffusionDrive matrix is camera$+$LiDAR only. The all-city camera-only rows in \reftab{tab:diffusiondrive-allcity} stand, but the city-disjoint camera-only matrix was not trained, so the thesis does not claim a camera-only cross-city generative result.
...
The most immediate next step is to move from image-based SSL to video-based SSL. Driving is inherently temporal, so a predictive video objective should align more naturally with planning than single-frame pretraining. Drive-JEPA~\cite{wang-drive-jepa} already suggests that V-JEPA~\cite{bardes-vjepa-2024} pretraining can be especially strong in-distribution, which makes it a natural candidate for the next iteration of the cross-city robustness question.

...
\begin{enumerate}
    ...
    \item \textbf{DiffusionDrive $+$ SSL}. SSL backbones are integrated into DiffusionDrive through the same feature adapter used in the cross-city benchmark, including a camera-only variant. The completed mixed-city results show that the representation signal survives a planner swap under pooled evaluation. The cross-city DiffusionDrive matrix is specified as active validation and is not used as completed evidence.
...
\begin{enumerate}
    ...
    \item \textbf{The in-distribution cost is small; the out-of-distribution gain is large.} The DiffusionDrive all-city table shows at most a small mixed-city cost for SSL, while the cross-city LAW and TransFuser / Latent TransFuser matrices show a materially larger transfer gain under explicit geographic shift. Whether this trade-off survives a planner swap under city-disjoint generative evaluation is active validation.
    ...
    \item \textbf{In the evaluated settings, representation choice mediates transfer more strongly than head choice.} The same ordinal pattern appears under LAW open-loop on nuScenes and TransFuser / Latent TransFuser closed-loop on NAVSIM, and the DiffusionDrive mixed-city integration preserves the representation-dependent structure at a small in-distribution cost. This is the basis for the bounded version of the central claim. The stronger claim, that the pattern is fully planner-invariant, depends on a completed cross-city DiffusionDrive matrix and remains active validation.
\end{enumerate}

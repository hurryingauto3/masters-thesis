...
\begin{enumerate}
    ...
    \item \textbf{DiffusionDrive $+$ SSL}. SSL backbones are integrated into DiffusionDrive through the same feature adapter used in the cross-city benchmark, including a camera-only variant. The mixed-city integration shows that the representation signal survives a planner swap under pooled evaluation. The completed $4 \times 4 \times 4$ city-disjoint matrix shows that it survives under city shift as well: every frozen self-supervised family produces a smaller mean out-of-distribution PDMS drop than supervised ResNet-34 (DINOv2 by $1.7$ points, MAE by $1.6$, I-JEPA by $3.3$), the per-training-city criterion pre-registered in \refsec{sec:meth-dd-success} is met, and the Singapore-trained row demonstrates that frozen SSL representations resist single-city overfitting under generative planning where the supervised baseline does not. The cross-city DiffusionDrive evidence is therefore treated as load-bearing rather than supportive.
...
\begin{enumerate}
    ...
    \item \textbf{In the evaluated settings, representation choice mediates transfer more strongly than head choice.} The same ordinal pattern appears under LAW open-loop on nuScenes, TransFuser and Latent TransFuser closed-loop on NAVSIM, and DiffusionDrive both in mixed-city pooled evaluation and in the completed $4 \times 4 \times 4$ city-disjoint matrix. The pattern survives the planner whose head was selected for its resistance to backbone variation, which is the strongest single piece of evidence for the planner-invariant version of the claim within the planners evaluated in this thesis.
\end{enumerate}

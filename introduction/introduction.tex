...
Once evaluation is re-framed as zero-shot geographic transfer, the picture changes. Naeinian et al.~\cite{naeinian-cross-city-2026} report that a supervised backbone trained on Boston and evaluated on Singapore inflates average trajectory displacement by $9.77\times$ and collision rate by $19.34\times$ in open-loop evaluation, while remaining competitive on the pooled split. In closed-loop NAVSIM evaluation, the same study reports that single-city training with self-supervised features improves average out-of-distribution PDMS by up to four points relative to supervised baselines. These failure modes are safety-critical: an autonomous vehicle trained in one city must remain competent when moved to another without expensive re-collection and re-labelling of local data.
...
The thesis began with a narrower question: whether a frozen self-supervised encoder paired with a small MLP waypoint regressor could already improve NAVSIM planning under mixed-city evaluation~\cite{hamza-sanchez-navsim-report}. Drive-JEPA~\cite{wang-drive-jepa} subsequently showed that V-JEPA-style video pre-training supports 93.3 PDMS on NAVSIM v1 and 87.8 EPDMS on NAVSIM v2, and that even a perception-free V-JEPA decoder reaches 89.0 PDMS. Mixed-city PDMS therefore became an insufficient discriminator for a representation-focused thesis. The scope shifted from \emph{whether} SSL helps mixed-city planning to \emph{where} it matters most and \emph{why}, with geographic transfer identified as the regime in which representation choice carries the most diagnostic weight.
...
\begin{enumerate}
    ...
    \item \textbf{Cross-city representation benchmark}. A controlled evaluation of self-supervised backbones (I-JEPA, DINOv2, MAE) against supervised ResNet-34 and Swin-T baselines across LAW on nuScenes and TransFuser / Latent TransFuser on NAVSIM. Under the evaluated settings, frozen self-supervised backbones reduce cross-city degradation relative to supervised baselines, with the largest open-loop effect being the reduction of Boston-to-Singapore L2 inflation from $9.77\times$ to $1.20\times$.
    \item \textbf{DiffusionDrive integration (mixed-city supporting breadth)}. Self-supervised backbones (I-JEPA, MAE) were integrated into the DiffusionDrive generative head under the same feature adapter used in the cross-city benchmark, including a camera-only variant. The completed mixed-city results ($86.9$ PDMS for MAE C+L, $86.6$ PDMS for supervised camera-only) show that the representation signal survives a planner swap under pooled evaluation, at approximately $1$ PDMS in-distribution cost relative to the supervised C+L baseline. This is reported as supporting breadth, not as completed cross-city evidence.
    \item \textbf{Mechanistic analysis}. A study of $188$ trained checkpoints using linear probing for city identity, centred kernel alignment, maximum mean discrepancy, and feature-covariance effective rank. Within the evaluated settings, the linear-probe ordering mirrors the out-of-distribution PDMS drop ordering across matched backbone families, and fine-tuning compresses feature-covariance rank for every self-supervised family; objective-specific nuance (strongest family, collapse frequency) is reported without claiming a universal mechanism.
\end{enumerate}

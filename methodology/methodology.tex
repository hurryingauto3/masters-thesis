...
The default lightweight head is the canonical diagnostic instrument and is worth writing out. Let $I_v$ denote the input image from view $v$, $\phi_\theta$ the visual backbone, and $f_\text{view}$ the view-fusion rule. The visual feature is
\begin{equation}
\label{eq:lh-visual}
...
DiffusionDrive's planner absorbs backbone differences by construction. The "head attenuates the backbone" claim is a precise consequence of the truncated denoising procedure, so it is worth writing down. The K-means anchor bank is fitted offline on the \texttt{navtrain} expert-trajectory pool by minimising
\begin{equation}
\label{eq:dd-kmeans}
...
\qquad n \in \{2,\,1\},
\end{equation}
where $\epsilon_\eta$ is the learned trajectory-residual network, $\alpha_n$ are the truncated-schedule coefficients of~\cite{jia-diffusiondrive-2025}, and $\mathrm{BEV}(\mathbf{s},\, \boldsymbol\tau_{\,n})$ is the backbone's contribution: a grid of bird's-eye-view features sampled around the current waypoints rather than a dense feature map. The conditioning enters $\epsilon_\eta$ only through this grid sample plus a time-only FiLM modulation on $n$. Four design choices make this attenuation explicit:
\begin{enumerate}
...
\noindent
This pre-registered criterion is what the interpretation of the result tables in \refsec{sec:exp-phase3} anchors on. \refsec{sec:exp-phase3} reports that the criterion is met on the completed $4 \times 4 \times 4$ matrix, in three of four training cities under the strictest per-backbone reading (DINOv2) and in four of four under the per-train-city reading. No architectural changes to DiffusionDrive were applied after the matrix was scored.

% ============================================================================
\section{Study Sequence}
\label{sec:meth-exp-protocol}

The experiments are organised as one exploratory study followed by four load-bearing studies. The exploratory lightweight-head study of~\ref{sec:meth-lightweight-head} motivates the representation-first framing. The cross-city SSL benchmark of~\ref{sec:meth-phase1-cc} produces the backbone-by-city matrix that Chapter~\ref{chap:experiments} reports. The generative-planning study adds DiffusionDrive with SSL backbones on the standard NAVSIM split, using the integration described in~\ref{sec:meth-diffusiondrive}. The cross-city generative study extends that integration to the city-disjoint setting; the completed $4 \times 4 \times 4$ matrix is reported in Chapter~\ref{chap:experiments} as load-bearing evidence, with the pre-registered criterion in~\ref{sec:meth-dd-success} met under the natural reading. The mechanistic analysis uses CKA, linear probing, MMD, and feature-covariance effective rank on the trained checkpoints, with no additional training.

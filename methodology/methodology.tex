...
The I-JEPA implementation is a fork of the official Meta release~\cite{assran-ijepa-2023}, vendored into the pretraining repository and adapted to consume the nuScenes pickle instead of ImageNet. The objective is exactly the one introduced in Section~\ref{sec:bg-ijepa}: predict target-region latent representations from visible context. The exact masking geometry, teacher setup, and optimizer recipe are tabulated in \reftab{tab:meth-ssl-methods}.

The target encoder is an EMA copy of the context encoder, and downstream consumers load that target-encoder state. The exact optimizer, schedule, and checkpoint-loading details are reported in \reftab{tab:meth-ssl-methods}; the prose point is that I-JEPA transfers the slowly averaged latent predictor rather than a pixel decoder or a supervised classification head.

\subsection{DINOv2: Self-Distillation Pretraining}
\label{sec:meth-ssl-dinov2}

The DINOv2 implementation is a fork of the official Meta release~\cite{oquab-dinov2-2024}, modified to accept the nuScenes pickle and to match the ViT-S/14 substrate. The objective is student--teacher self-distillation with multi-crop augmentation. Patch-level iBOT loss is disabled because its pretraining benefit does not transfer cleanly to the small driving corpus; the exact crop, temperature, EMA, and optimizer settings are in \reftab{tab:meth-ssl-methods}.

Optimization and checkpoint serialization are tabulated in \reftab{tab:meth-ssl-methods}. The important comparison point is that DINOv2 transfers the teacher backbone from a student--teacher distillation run; the downstream planner never sees the student wrapper or the prototype head.

\subsection{MAE: Masked Reconstruction Pretraining}
\label{sec:meth-ssl-mae}

The MAE implementation is a fork of the official Meta release~\cite{he-mae-2022}, with a \texttt{Masked\-Autoencoder\-ViT} variant adapted for the rectangular input path. The objective is the canonical masked-patch pixel-reconstruction task: visible patches pass through the encoder, and a lightweight decoder reconstructs the masked pixels. The masking rate, decoder dimensions, and optimization details are in \reftab{tab:meth-ssl-methods}.

Optimization and checkpoint loading are summarized in \reftab{tab:meth-ssl-methods}. The important distinction from I-JEPA and DINOv2 is architectural: MAE has no EMA teacher at all, so downstream transfer extracts the encoder portion of a single reconstruction model.
